...
export const publicConfiguration = () => {
  ...
  return {
    ...
    wallet: {
      configured: config.wallet.liveReady,
      chainId: config.wallet.liveReady ? config.wallet.chainId : null,
      gas: config.wallet.liveReady ? "sponsored-supported-actions" : "disabled"
    },
    auctions: {
      configured: config.auctions.liveReady,
      rails: config.auctions.liveReady ? ["card"] : []
    },
