...
export const requireSecondarySponsorshipConfig = (config = getRuntimeConfig()) => {
...
export const resolveSecondaryActionContext = async ({ service, config, userId, account, body }) => {
  ...
  if (["resale-approve-token", "resale-revoke-token"].includes(action)) {
    ...
    const [{ data: collection, error: collectionError }, { data: owner, error: ownerError }, unresolved] = await Promise.all([
      service.from("nft_collections").select("id,standard,chain_id,contract_address,deployed_code_hash,state")
    ...
    return {
      ...
      reference: { work_id: work.id, collection_id: collection.id, token_id: String(work.nft_token_id) }
    };
  }
...
export const simulateSecondaryAction = async ({ config, decoded, expected, client = publicClientFor(config) }) => {
  ...
  if (["resale-approve-token", "resale-revoke-token"].includes(decoded.action)) {
...
const verifyActionReceiptLog = ({ config, decision, result }) => {
...
export const reconcileSponsoredSecondaryOperation = async ({ service, config, decision, provider, client = publicClientFor(config) }) => {
  ...
  if (!["finalized", "failed"].includes(result.state)) {
    ...
    if (result.state === "pending" && canonical
      ...
      if (result.state === "pending") await replaySubmittedOperation({ config, decision, adapter });
  ...
  if (orderEvent) {
    ...
    const eventRecord = {
      ...canonicalEvent,
      resale_order_id: decision.policy_input.reference.listing_id,
      chain_id: config.wallet.chainId,
      provider: "configured-rpc"
    };
    let inserted = await service.from("chain_event_inbox").insert(eventRecord).select("id").maybeSingle();
    if (inserted.error?.code === "23505") inserted = await service.from("chain_event_inbox").select("id")
...
export const recordRejectedSecondarySponsorship = async ({ service, config, userId, accountId, body, error }) => {
  ...
  if (!/^(INVALID_|SECONDARY_(CALL|SELF|TOKEN|ORDER|LISTING|SELLER|BUYER|REVOKE|ACTION|USDC|FULFILLMENT))/.test(rawCode)) return;
