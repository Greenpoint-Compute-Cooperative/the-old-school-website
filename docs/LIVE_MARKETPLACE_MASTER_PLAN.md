...
| Artwork delivery | Vercel Image Optimization + content-addressed Supabase Storage | bounded AVIF/WebP display variants; immutable IPFS remains token authority |
| Public market stats | Postgres finalized snapshot | returns syncing instead of incomplete/stale totals |
| Instagram intake | signed Meta webhook + private Postgres inbox | explicit commands create rights-unverified drafts only |
